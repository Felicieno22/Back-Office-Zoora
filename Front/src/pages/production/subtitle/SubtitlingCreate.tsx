...
export function SubtitlingCreate({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiRequest('/api/soustitres', {
        method: 'POST',
        data: payload
      });

      toast.success('Sous-titrage créé avec succès !');
      navigate('/admin/production/subtitles');
    } catch (error) {
      console.error('Erreur création sous-titrage:', error);
      toast.error('Erreur lors de la création du sous-titrage');
    }
  };
  ...
  return (
    <PageContainer>
      ...
      >
        <h1 className="text-2xl font-bold mb-2">Créer un nouveau sous-titrage</h1>
        <p className="text-sm text-gray-600 mb-4">Ajoutez des sous-titres pour un contenu ou un épisode spécifique</p>
      </ThemedCard>
